...
def format_minor_ticks(x, pos):
    if x == 0:
        return ""
    # Format numbers like 20000 as 2×10⁴
...
# Set up minor ticks for y-axis to show more labels
...
legend_elements_bounds = [
    Line2D([0], [0], color='black', lw=2, linestyle='-', 
           label='T$_{ideal}$ (perfect overlap)'),
    Line2D([0], [0], color='black', lw=2, linestyle='--', 
           label='T$_{sequential}$ (no overlap)'),
...
first_legend = ax.legend(handles=legend_elements_bounds, loc='lower right', title='Data Type')
ax.add_artist(first_legend)

# Create second legend for radii
ax.legend(handles=legend_elements_radii, loc='lower right', title='Radius', bbox_to_anchor=(0.7, 0))
...
# Save the plot
plt.tight_layout()
plt.savefig('throughput_comparison_wse3_presentation.png', dpi=300, bbox_inches='tight')
print("Plot saved as 'throughput_comparison_wse3_presentation.png'")
